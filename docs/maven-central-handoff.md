...
2. **Publish** using existing Kill Bill infrastructure:
   ...
   - `mvn release:clean release:prepare release:perform` (see [killbill-avatax-plugin release workflow](https://github.com/killbill/killbill-avatax-plugin/blob/master/.github/workflows/release.yml))
